// Pipeline del tema "expresiones / modismos" del examen Lexis IT.
...
for (const it of SRC) {
  ...
  // No exigimos que la frase contenga la expresión literal (en IT la locución
  // se flexiona); el motor usa el subtipo "hueco" solo si coincide, si no, "significado".
  if (seen.has(pv)) continue; seen.add(pv);
  out.push({ pv, cefr, b: cefrToB(cefr,pv), es, sent, ds, def: norm(it.def||"") });
}
out.sort((a,b)=>a.b-b.b);
const js = "// Tema 'expresiones / modismos' del examen Lexis IT — generado por gen-examen-phrasal.mjs\n" +
...
console.log("=== Lexis IT · expresiones ===");
